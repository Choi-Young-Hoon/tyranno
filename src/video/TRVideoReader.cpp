...
void TRVideoReader::Open(std::string& file_name, TRError* error) {
    Open(file_name.c_str(), error);
}

void TRVideoReader::Open(const char* file_name, TRError* error) {
...
bool TRVideoReader::ReadFrame(TRFrame* frame, TRError* error) {
    if (!IsOpen()) {
...
uint TRVideoReader::GetStreamsCount() {
    if (IsOpen() == false) {
        return 0;
    }
...
TRCodecID TRVideoReader::GetCodecID() {
...
TRStreamIndex TRVideoReader::GetStreamIndex() {
    return this->stream_index_;
}

TRCodecParameters TRVideoReader::GetCodecParameters() {
...
TR_CODEC_ID TRVideoReader::GetCodecID(int stream_index) {
...
AVCodecParameters* TRVideoReader::GetAVCodecParameters(int stream_index) {
    if (IsOpen() == false || stream_index < 0) {
        return NULL;
    }
